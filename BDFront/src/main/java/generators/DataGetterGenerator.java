...
public class DataGetterGenerator {
    ...
    public static Entity createGroupDataGetter(Component component, JSONConnection connection, Vector<Object> lastArgs) {
        ...
        if (!isGettingEquipmentType) {
            JButton typesButton = new JButton("add group type");
            typesButton.addActionListener(action -> {
        ...
        buffArray = ListableFields.getCompanies().toArray();
        sBuffArray = new String[buffArray.length];
        for(int i = 0 ; i < buffArray.length ; i ++){
            try {
                sBuffArray[i] = buffArray[i].toString();
        ...
        JComboBox<String> companies = new JComboBox<String>(sBuffArray);
        messages.add(companies);
        companies.setVisible(false);
        JButton companiesButton= new JButton("add company");;
        if(!isGettingCompany){
            companiesButton.addActionListener(action -> {
                Entity subEntity = createCompanyDataGetter(component, connection, null);
                if (subEntity != null) {
                    companies.removeAllItems();
                    ListableFields.getCompanies().forEach(type -> {
                        group_types.addItem(type);
                    });
                    companies.repaint();
                }
            });
            messages.add(companiesButton);
            companiesButton.setVisible(false);
        }
        group_types.addActionListener(action->{
            String type = (String)group_types.getSelectedItem();
            if(type.equals("company")){
                ...
                companiesButton.setVisible(true);
...
//todo change
        ...
        if (res == JOptionPane.OK_OPTION) {
            ...
            if (lastArgs != null) {
                newId= groupId;
                if(!group_type.equals((String)group_types.getSelectedItem())){
                    Vector<MyEntry<String, Vector<String>>> newType = new Vector<>();
                    Vector<String> b = new Vector<>();
                    b.add((String)group_types.getSelectedItem());
                    newType.add(new MyEntry<>("group_type",b));
                    JSONString = StringJSONMessageGenerator.replaceEntriesTableMsg("working_groups","group_id",String.valueOf(groupId),newType);
                    String result =connection.makeRequest(JSONString);
                    MyEntry<Boolean,Integer> resId;
                    try {
                        resId = JSONResponseProcessor.processId(result);
                    } catch (ParseException e) {
                        JOptionPane.showMessageDialog(component, "Error result format", "Server error", JOptionPane.ERROR_MESSAGE);
                        isGettingGroup = false;
                        return null;
                    }
                    if(((String)group_types.getSelectedItem()).equals("company")){
                        JSONString = StringJSONMessageGenerator.deleteTableEntryMsg("groups","group_id",String.valueOf(newId));
                ...
                JSONString = StringJSONMessageGenerator.addTableEntryMsg("groups_bind", subArgs);
                String newIdStringBuffer = connection.makeRequest(JSONString);
                MyEntry<Boolean, Integer> idResult;
                try {
                    idResult = JSONResponseProcessor.processId(newIdStringBuffer);
                ...
                if(((String)group_types.getSelectedItem()).equals("company")){
                    Vector<MyEntry<String,String>> ssubArgs =new Vector<>();
                    ssubArgs.add(new MyEntry<>("group_id",String.valueOf(newId)));
                    ssubArgs.add(new MyEntry<>("company_name",(String)companies.getSelectedItem()));
                    JSONString = StringJSONMessageGenerator.addTableEntryMsg("working_company", ssubArgs);
                    String adding_gc = connection.makeRequest(JSONString);
                    MyEntry<Boolean, Integer> gc_res;
                    try {
                        idResult = JSONResponseProcessor.processId(newIdStringBuffer);
                    } catch (ParseException e) {
                        JOptionPane.showMessageDialog(component, "Error result format", "Server error", JOptionPane.ERROR_MESSAGE);
                    ...
                    newArgs.add(new MyEntry<>("group_id", buffer));
                    JSONString = StringJSONMessageGenerator.addSerialTableEntryMsg("groups", newArgs);
                    boolean successful = connection.makeStatement(JSONString);
                    if (successful) {
                        JOptionPane.showMessageDialog(component, "New equipment group added. Id: " + String.valueOf(newId));
                        groupId = newId;
                        entity.addField("group_id", newId, "INTEGER");
                        entity.addField("worker_ids", valuesBuffer, "INTEGER");
                    } else {
                        JOptionPane.showMessageDialog(component, "Error at creating equipment group", "Error", JOptionPane.ERROR_MESSAGE);
                        entity = null;
                    }
    ...
    public static Entity createGroupTypeDataGetter(Component component, JSONConnection connection, Vector<Object> lastArgs){
        ...
        if (res == JOptionPane.OK_OPTION) {
            Vector<MyEntry<String, String>> newArgs = new Vector<>();
            newArgs.add(new MyEntry<>("group_type", textField.toString()));
            String JSONString;
            if (lastArgs != null) {
            ...
            if (result) {
                ...
                entity.addField("group_type", textField.toString(), "TEXT");
                ListableFields.addGroupType(textField.toString());
            } else {
                if (lastArgs != null) {
    ...
    public static Entity createCompanyDataGetter(Component component, JSONConnection connection, Vector<Object> lastArgs) {
